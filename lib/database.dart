...
class DatabaseHelper {
  ...
  Future _onCreate(Database db, int version) async {
    ...
    String medias = "create table if not exists medias(id integer, media varchar(200) not null)";
  ...
  select_prodotti() async{
    Database db = await instance.database;
    var rows = await db.rawQuery("select distinct fotos, cantieri from prodotto order by fotos");
    return rows;
  }
  select_prodotti_based_on_id(String fotos) async{
    Database db = await instance.database;
    var result = await db.rawQuery("select * from prodotto where fotos='"+fotos.toString()+"'");
    print(result);
    return result;
  }
  deleteProdotti(String id) async{
    Database db = await instance.database;
    try{
      await db.rawQuery("delete from prodotto where fotos ='"+id.toString()+"'");
    }catch(e){
      print(e);
    }
  }
  ...
  insert_medias(String media) async{
    Database db = await instance.database;
    var id = await select_id_medias();
    if(id == null){
      id = 1;
    }
    var res = "insert into medias(id, media) values ('"+id.toString()+"','"+media.toString()+"');";
    var result =  await db.rawQuery(res);
    return result;
  }
  deletemedias(int id) async{
    Database db = await instance.database;
    return await db.rawQuery("delete from medias where id='"+id.toString()+"'");
  }
  select_medias() async{
    Database db = await instance.database;
    final res = await db.query("medias");
    return res;
  }
  create_medias() async{
    Database db = await instance.database;
  ...
  drop_medias() async{
    Database db = await instance.database;
    return await db.rawQuery("delete from medias");
  }
